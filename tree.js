...
(function() {
  ...
  window.Graph = (function() {
    ...
    Graph.prototype.addNode = function(data, parent, index) {
      var node;
      node = new Graph.Node;
      ...
      if (parent != null) {
        this._link(node.id, parent, index);
      }
      return node;
    };

    Graph.prototype.moveNode = function(id, newparent, index) {
      var oldparent;
    ...
    Graph.prototype.isDescendent = function(pid, cid) {
      ...
      return this.isDescendent(pid, this.node(cid).parent);
    };

    Graph.prototype.isConnected = function(id1, id2) {
      return this.isDescendent(id1, id2) || this.isDescendent(id2, id1);
    };

    Graph.prototype.isCycle = function(id) {
      var cycle,
        _this = this;
      cycle = function(id, list) {
        if (id == null) {
          return false;
        }
        ...
        list.push(id);
        return cycle(_this.node(id).parent, list);
      };
      return cycle(id, []);
    };
    ...
    Graph.prototype._link = function(child, parent, index) {
      if (this.node(parent) == null) {
        throw "Parent node " + parent + " does not exist.";
      }
      this.node(child).parent = parent;
      if (index == null) {
        index = this.node(parent).children.length;
      }
      this.node(parent).children.splice(index, 0, child);
  ...
  window.Graph.vis = (function() {
    ...
    vis.prototype.drawAux = function(aux) {
      var aux_lines;
      aux_lines = d3.select("" + this.div + " .g_lines").selectAll(".aux").data(aux, function(d) {
        return d.id;
      });
      aux_lines.enter().append("line").attr("class", "aux").attr("data-id", function(d) {
        return d.id;
      }).attr("x1", function(d) {
        return d.d3.parent.oldx;
      }).attr("y1", function(d) {
        ...
        return d.d3.child.oldx;
      }).attr("y2", function(d) {
        return d.d3.child.oldy;
      });
      aux_lines.transition().duration(0).attr("x1", function(d) {
        return d.d3.parent.x;
      }).attr("y1", function(d) {
        return d.d3.parent.y;
      }).attr("x2", function(d) {
